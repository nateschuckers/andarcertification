...
const QuestionView = ({ course, user, onBack, trackIcon }) => { // Accept trackIcon prop
    ...
    const handleCancelExit = () => { window.history.pushState(null, ''); setShowExitConfirm(false); };
        ...
        <div className="relative">
            ...
            <div className={`p-8 max-w-4xl mx-auto ${showExitConfirm || selectedAnswer !== null ? 'blur-sm' : ''}`}>
                <div className="bg-neutral-100 dark:bg-neutral-800 rounded-lg p-8 shadow-inner dark:shadow-neutral-950">
                    <h2 className="text-3xl font-bold mb-4 text-neutral-900 dark:text-white flex items-center">
                        {trackIcon && <i className={`fa ${trackIcon} mr-4 text-blue-500`}></i>}
                        {course.title}
                    </h2>
                    <div className="text-center mb-4 p-2 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded-md text-sm font-semibold">A score of {PASS_RATE * 100}% is required to pass.</div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {currentQuestion.options.map((option, index) => <button key={index} onClick={() => handleAnswerClick(index)} disabled={selectedAnswer !== null} className={getButtonClass(index)}>{option}</button>)}
                        </div>
                    </div>
                </div>
            </div>
